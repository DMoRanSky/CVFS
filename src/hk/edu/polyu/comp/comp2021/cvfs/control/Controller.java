    ...
    public void run() {
        ...
        while (true) {
            ...
            try {
                ...
                if (getS().getD() == null && (!c.equals("newDisk"))) {
                    throw new Exception("you should write: 'newDisk v' (v must be a number) to first create a disk");
                }

                if (c.equals("newDisk")) {
                    err = "Error! you should write: 'newDisk v' (v must be a number)";
                    int v = sc.nextInt();
                    getS().setD(new Disk(v));
                    getUn().clear();
                    getRe().clear();
                } else if (c.equals("newDoc")) {
                    err = "Error! you should write: 'newDoc name type content'";
                    ...
                    err = "Error! you should write: 'newSimpleCri criterionName attributeName operator value'";
                    String cn = sc.next(), an = sc.next(), op = sc.next(), val = sc.next();
                    push(new NewSimpleCri(cn, an, op, val, getS()));
                } else if (c.equals("newNegation")) {
                    err = "Error! you should write: 'newNegation c1Name c2Name'";
                    err += "\n(c1 = !c2)";
                    String c1 = sc.next(), c2 = sc.next();
                    push(new NewNeg(c1, c2, getS()));
                } else if (c.equals("newBinaryCri")) {
                    err = "Error! you should write: 'newBinaryCri c1Name c2Name operator c3Name'";
                    err += "\n(c1 = c2 op c3)";
                    ...
                    err = "Error! you should write: 'search criterionName'";
                    String n = sc.next();
                    Cri cr = getS().getCri(n);
                    getV().list(getS().getD().getCwd(), cr);
                } else if (c.equals("rSearch")) {
                    err = "Error! you should write: 'rSearch criterionName'";
                    String n = sc.next();
                    Cri cr = getS().getCri(n);
